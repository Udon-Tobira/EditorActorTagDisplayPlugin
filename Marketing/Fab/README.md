...
1. `final/01-actor-metadata-overlay-hero.jpg` — `ACTOR METADATA OVERLAY`; a Deep Water Station viewport with four real rule-matched overlays and a zone bounding box.
...
The left-bottom disclaimer `Demo scene not included.` appears in every final image. No marketplace badges, prices, arrows, fabricated Unreal UI, or AI-generated imagery are used.
...
The outer labels, cards, gradient, and token example are deterministic presentation layers around real Unreal Engine captures. `Demo scene not included.` is a disclosure, not a product feature claim.
